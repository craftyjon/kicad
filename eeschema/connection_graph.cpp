...
int CONNECTION_GRAPH::MigrateBusesWithMultipleLabels()
{
    ...
    for( auto it = m_subgraphs.begin(); it < m_subgraphs.end(); it++ )
    {
        ...
        for( auto item : subgraph->m_drivers )
        {
            ...
            case SCH_GLOBAL_LABEL_T:
            {
                auto label_conn = item->Connection( sheet );

                // Only consider bus vectors
                if( label_conn->Type() == CONNECTION_BUS )
                    label_drivers.push_back( static_cast<SCH_TEXT*>( item ) );
            }
            default: break;
            }
        }
        ...
        SCH_TEXT* label_to_keep = nullptr;

        if( label_drivers.size() > 1 )
        {
            std::cout << "Bus with multiple drivers: SG " << subgraph->m_code << std::endl;

            for( auto label : label_drivers )
            {
                SCH_CONNECTION conn;
                ...
                std::cout << label->GetText() << " " << start << " " << end << std::endl;
            ...
            std::cout << "Merged vector: [" << highest_end << ".." << lowest_start << "]" << std::endl;
            std::cout << "Widest label: " << label_to_keep->GetText() << " width " << widest_bus << std::endl;

            SCH_CONNECTION conn;
            conn.ConfigureFromLabel( label_to_keep->GetText() );

            auto new_label_text = conn.VectorPrefix();
            new_label_text << "[" << highest_end << ".." << lowest_start << "]";

            label_to_keep->SetText( new_label_text );
            ...
            for( auto label : label_drivers )
            {
                ...
                {
                    // TODO(JE) Is this the best way to delete? Do we need
                    // access to the frame from this object?
